...
-(ImagesView *)images
{
    if (!_images) {
        ...
        _images.backgroundColor = [UIColor yellowColor];
    }
    return _images;
}
...
-(void)SetUpUI
{
    ...
    _imageScrollView.backgroundColor = [UIColor blueColor];
    
    for (int i = 0; i <5; i++) {
        UIButton *button = [[UIButton alloc]init];
        
        button.frame = CGRectMake((margin/2 + ButtonW)*i,0, ButtonW, ButtonW);
...
-(void)setImagesArray:(NSMutableArray *)imagesArray
{
    ...
    if (imagesArray.count == 0) {
        ...
        [self mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            make.height.mas_equalTo(ButtonW).priorityMedium();
            
        }];
    }
    _imageScrollView.contentSize=CGSizeMake((margin/2 + ButtonW) * imagesArray.count,ButtonW);
